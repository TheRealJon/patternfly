...
The packages listed below are no longer supported and will not include any features or fixes introduced after Patternfly 3.23.  
...
This task will compile and minify the lesscss files into CSS files located at `dist/css/patternfly.min.css` and `dist/css/patternfly-additions.min.css`.

### Less to Sass Conversion
Any time style changes are introduced, the Sass will need to be updated to reflect those changes. This can be accomplished by adding a `--sass` flag to the tasks above:
